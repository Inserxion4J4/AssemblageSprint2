...
public class explosionGrenade : MonoBehaviour
{
    ...
    void Explosion()
    {
        GameObject explosionClone = Instantiate(effetExplosion, transform.position, transform.rotation);
        explosionClone.SetActive(true);
        Destroy(explosionClone, 4f);
        Destroy(gameObject);
        ...
        foreach (Collider objetsProches in collisions)
        {
            ...
            if (rb != null)
            {
                ...
                if (objetsProches.gameObject.CompareTag("ennemi"))
                {
                    ...
                    ennemiScript.compteurEnnemisMorts += 1;
                }
            }
        }
    }
