...
public class MouvementPerso : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ennemiScript.compteurEnnemisMorts = 0;
    }
    private void Awake()
    {
    ...
    void Update()
    {
        ...
        animator.SetFloat("x", movementMagnitude == 0 ? 0 : -1);
        animator.SetFloat("y", 0);

        if (Input.GetKeyDown(KeyCode.Mouse1))
        {
            gameObject.GetComponent<Animator>().SetBool("viseeActive", true);
        }
        if (Input.GetKeyUp(KeyCode.Mouse1))
        {
            gameObject.GetComponent<Animator>().SetBool("viseeActive", false);
        }
        ...
        animator.SetFloat("vitessePerso", speed);
